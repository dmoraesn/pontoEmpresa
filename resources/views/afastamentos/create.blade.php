...
@section('title', 'Novo Afastamento')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form action="{{ route('afastamentos.store') }}" method="POST">
            @csrf

            {{-- Usuário --}}
            <div class="mb-4">
                <label for="usuario_id" class="block text-sm font-medium text-gray-700">Funcionário</label>
                <select name="usuario_id" id="usuario_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="mb-6">
                ...
                <textarea name="observacao" id="observacao" rows="3"
                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('observacao') }}</textarea>
                @error('observacao')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
